...
while True:
	...
	if len(serious)>2 or len(abnormal)>2:
		...
		#xi, yi = np.mgrid[px.min():px.max():px.size**0.5*1j,py.min():py.max():py.size**0.5*1j]
		...
		#ax2.set_xlim(px.min(), px.max())
		#ax2.set_ylim(py.min(), py.max())

		# you can also overlay your soccer field
		#ax1.imshow(im, extent=[px.min(), px.max(), py.min(), py.max()], aspect='auto')
		#ax2.imshow(im, extent=[px.min(), px.max(), py.min(), py.max()], aspect='auto')
		
		#ax.scatter(px, py, c=z, s=100)
		
		#heatmap, xedges, yedges = np.histogram2d(px, py, bins=100)
		#extent = [xedges[0], xedges[-1], yedges[0], yedges[-1]]

		#ax.pcolormesh(px, py, z, cmap='rainbow')
